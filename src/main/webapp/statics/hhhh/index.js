$(document).ready(function(){
		...
		$.get("http://localhost:8081/data/pathSelect",function(data){
			...
			for(var i=0 ;i< app.length ;i++){
				...
				$('#slpkApp').css("tabindex","100");
				console.log(option);
                console.log($('#slpkApp')[0]);
			...
			$('#slpkPath').selectpicker('refresh');
            $('#slpkApp').selectpicker('refresh');
		});
		...
		$("#clear").click(function(){
 		...
 		$("#query").click(function(){
 			...
 			var url = "http://localhost:8081/data/pathDataList?startDate="+startDate+"&endDate="+endDate+"&appId="+appId+"&companyKey="+companyKey;
 			console.log(url);
 			$.get(url,function(data){
				...
				$(r).append(d9);
				$(d10).text(one.toPathTrueCount/one.fromPathCount);
				$(r).append(d10);
				$("#pathData").append(r);
